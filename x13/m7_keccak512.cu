...
static __constant__ const uint64_t RC[24] = {
...
__global__ void  m7_keccak512_gpu_hash_120(int threads, uint32_t startNounce, uint64_t *outputHash)
{
  ...
  if (thread < threads)
  {
    ...
    #pragma unroll
		for (int i=0;i<25;i++) state[i] = stateo[i];
		((uint2*)state)[5].y ^= nounce;
...
//	cudaMemcpyToSymbol( c_PaddedMessage80, PaddedMessage, 16*sizeof(uint64_t), 0, cudaMemcpyHostToDevice);
	...
	for (int i=0;i<7;i++) state[i] ^= ((uint64_t*)PaddedMessage)[i + 9];
	((uint2*)state)[5].y ^= ((uint2*)PaddedMessage)[5 + 9].y;
...
__host__ void m7_keccak512_cpu_hash(int thr_id, int threads, uint32_t startNounce, uint64_t *d_hash, int order)
{
  ...
  size_t shared_size = 0;
	if (compute_version[thr_id]<35) m7_keccak512_gpu_hash_120 <<< grid, block, shared_size >>>(threads, startNounce, d_hash);
	else m7_keccak512_gpu_hash_120_v35 <<<grid, block, shared_size >>>(threads, startNounce, d_hash);
